...
class SymbolicEngine:
        ...
        for base_path_tx_seqs_map in all_tx_sequences.values():
            for base_path in base_path_tx_seqs_map.keys():
                ...
                for one_seq in tx_seq_set:
                    ...
                    # 输出执行结果
                    self.log_symbolic_execution_res(is_secure, one_seq, perm_nodes, init_storage)
    ...
    def exec_one_tx_sequence_with_constructor(self, sliced_graph: SlicedGraph, init_storage: MemoryModel, tx_sequence: TxSequence):
        ...
        if constructor is not None:
            ...
            entry_state: SymbolicState = SymbolicState(solver=self.solver, tx=constr_tx, init_storage=init_storage,
    ...
    def log_symbolic_execution_res(self, is_secure: bool, txs: TxSequence, perm_nodes: List[ICFGNode], storage: MemoryModel) -> None:
        print(f"{YELLOW}==================================== GALA CHECK RESULT ===================================={RESET}")
        # 输出攻击者和部署者的地址
        ...
        if is_secure:
            ...
            for var in model:
                var_value = model[var]
                if isinstance(var_value, BitVecNumRef) and var_value.sort().size() == 160:  # 地址类型
                    var_value_hex = hex(var_value.as_long())
                    print(f"{var.name()} = {var_value_hex}")
                else:
        ...
        for state_var in storage.MU.keys():
            ...
            if isinstance(sym_state_val, BitVecNumRef) and sym_state_val.sort().size() == 160:  # 地址类型
