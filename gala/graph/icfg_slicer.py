...
class ICFGSlicer:
    ...
    def slice_one_func_paths(sliced_graph: SlicedGraph, func_entry: ICFGNode) -> None:
        ...
        work_list: List[Tuple[ICFGNode, List[ICFGNode], List[ICFGNode], List[ICFGNode], List[ICFGNode]]] = \
            [(func_entry, list(), list(), list(), list())]

        while len(work_list) > 0:
            cur_node, visited_nodes, req_nodes, perm_nodes, call_stack = work_list.pop()
            ...
            if len(call_edges) > 0:  # 说明当前是函数调用节点
                ...
                work_list.append(
                    (dst_node, visited_nodes.copy(), req_nodes.copy(), perm_nodes.copy(), call_stack.copy()))
            else:
                ...
                control_flow_out_edges = EdgeProcessor.get_edges_by_types(graph, cur_node, control_flow_edge_types,
                    ...
                    if len(call_stack) == 0:
                        ...
                        for vn in visited_nodes:
                            if graph.nodes[vn]["is_requirement_node"]:
                                ...
                                sliced_graph.perm_node_slice_map.setdefault(vn, dict()).setdefault(one_slice_path,
                                                                                                   req_nodes.copy())
                                ...
                                if perm_info.type == PermissionType.MODIFY_STATE:
                                    ...
                                    (sliced_graph.state_var_write_slice_map.setdefault(state_written, dict()).
                                     setdefault(vn, list()).append(one_slice_path))
                        # 将路径上的所有约束节点进行保存
                        ...
                        for next_edge in return_site_next_edges:
                            ...
                            work_list.append(
                                (dst_node, visited_nodes.copy(), req_nodes.copy(), perm_nodes.copy(),
                                 call_stack.copy()))
