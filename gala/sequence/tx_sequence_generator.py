...
from typing import Dict, List, Tuple, FrozenSet, Set, TypeAlias
...
class TxSequenceGenerator:
    ...
    def generate_one_perm_node_tx_sequence(self, sliced_icfg: SlicedGraph, base_path: SlicedPath, perm_req_nodes: List[ICFGNode]) -> Set[TxSequence]:
        ...
        for req_node in perm_req_nodes:
            ...
            for dependent_state_var in req.taint_result[base_path].state_vars_flow_to_sink:
                if dependent_state_var not in perm_req_nodes_dependent_state_vars:
        ...
        for write_first_sv_node_and_slice in write_first_sv_slices:
            work_list.append(
                (write_first_sv_node_and_slice[1],  # working_slice
                 ...
                 set()))  # state_vars_processed

        # 4.外部大循环，生成Tx Seq
        # TODO 在constructor函数中有些状态变量已经被写，且只被写了一次，这些也应该考虑
        # ERROR：有一些状态变量可能没有写操作，同样会KeyError
        while len(work_list) > 0:
            ...
            # TODO KeyError: <gala.graph.sliced_graph.SlicedPath object at 0x10781d810>
            ...
            if len(dependent_state_vars_not_processed) == 0:
                ...
                state_var_write_in_slice = self.get_sv_write_nodes_modify_state_vars(icfg, working_slice.sv_write_nodes)
                # 将当前Slice路径上写入的变量加入到已经处理的集合中
                state_vars_processed.update(state_var_write_in_slice)
    ...
    @staticmethod
    def get_candidate_slices(sliced_icfg: SlicedGraph, state_var_dependent: StateVariable) -> List[
        Tuple[ICFGNode, SlicedPath]]:
    ...
    @staticmethod
    def get_sv_write_nodes_modify_state_vars(icfg: ICFG, sv_write_nodes: List[ICFGNode]) -> Set[StateVariable]:
        modify_state_vars: Set[StateVariable] = set()
        for node in sv_write_nodes:
